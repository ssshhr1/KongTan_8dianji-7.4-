...
//————————————————————————————————————————
std::set<CML::uint> MotorIds = { 1, 2, 3, 4, 5, 6, 7, 8 }; // 电机ID
//————————————————————————————————————————
...
BOOL CKongTan8dianjiDlg::OnInitDialog()
{
    ...
    // Opening the application never connects/enables hardware.
...
void CKongTan8dianjiDlg::OnBnClickedButton1()  //S1-上
{
	...
	// 电机1正转，电机2反转
	if (SetLegacyVelocity(1, motor_V12, 200000, 200000))
	{
		MotorMoveRel(1, motor_S12);
	}

	if (SetLegacyVelocity(2, motor_V12, 200000, 200000))
	{
...
void CKongTan8dianjiDlg::OnBnClickedButton4()  //S1-左
{
	...
	// 电机3正转，电机4反转
	if (SetLegacyVelocity(3, motor_V34, 200000, 200000))
	{
		MotorMoveRel(3, motor_S34);
	}

	if (SetLegacyVelocity(4, motor_V34, 200000, 200000))
	{
...
void CKongTan8dianjiDlg::OnBnClickedButton7()  //S2-勾
{
	...
	// 电机5正转，电机6反转
	if (SetLegacyVelocity(5, 2.5 * motor_V56, 200000, 200000))
	{
		MotorMoveRel(5, 2.5 * motor_S56);
	}
	if (SetLegacyVelocity(6, motor_V56, 200000, 200000))
	{
		MotorMoveRel(6,-motor_S56);
	}
	if (SetLegacyVelocity(1, 1.5 / 1.1 * motor_V56, 200000, 200000))//M1
	{
		MotorMoveRel(1, 1.5 / 1.1 * motor_S56);
	}
	if (SetLegacyVelocity(2, 4.25 / 1.5 * motor_V56, 200000, 200000))//M2
	{
		MotorMoveRel(2, -4.25 / 1.5 * motor_S56);
	}
	if (SetLegacyVelocity(3, 2.75 / 1.5 * motor_V56, 200000, 200000))//M3
	{
		MotorMoveRel(3, -2.75 / 1.5 * motor_S56);
	}
	if (SetLegacyVelocity(4, 0.34 / 1.5 * motor_V56, 200000, 200000))//M4
	{
...
void CKongTan8dianjiDlg::OnBnClickedButton8()  //S2-回
{
	...
	// 电机5反转，电机6正转
	if (SetLegacyVelocity(5, motor_V56, 200000, 200000))
	{
		MotorMoveRel(5, -motor_S56);
	}

	if (SetLegacyVelocity(6, 3 * motor_V56, 200000, 200000))
	{
...
void CKongTan8dianjiDlg::OnBnClickedButton10()  //S3-俯
{
	...
	// 电机7正转，电机8反转
	if (SetLegacyVelocity(7, motor_V78, 200000, 200000))
	{
		MotorMoveRel(7, motor_S78);
	}

	if (SetLegacyVelocity(8, motor_V78, 200000, 200000))
	{
...
void CKongTan8dianjiDlg::OnBnClickedButton13()  //全部复位
{
	...
	for (CML::uint motorId = 1; motorId <= 8; ++motorId)
	{
	...
	MessageBox(_T("八个机器人电机复位流程结束；进给轴请单独回起点！"), _T("提示"), MB_ICONINFORMATION);
}
...
void CKongTan8dianjiDlg::ExecuteBendingMotion(double bend_angle3_deg, double bend_angle4_deg, double plane_angle_deg)
{
	...
	if (!motors.empty() && maxDist >= 0.5)
	{
		for (auto& m : motors)
		{
			...
			m.accel = 200000.0;
			SetLegacyVelocity(m.id, m.speed, m.accel, m.accel);
		}
		...
		for (auto& m : motors)
		{
			...
			if (!WaitForMotorMotion(m.id, (float)timeoutMs)) return;
		}
	}
...
void CKongTan8dianjiDlg::OnBnClickedCcancel()
{
    OnCancel();
}
...
void CKongTan8dianjiDlg::SyncedTrapezoidalReset(const std::vector<int>& motorIDs)
{
	...
	for (auto& m : motors)
	{
		SetLegacyVelocity(m.id, m.velocity, m.accel, m.accel);
	}

	// ── 第5步：再同时发出所有电机的绝对定位指令（确保同时启动）──
	for (auto& m : motors)
	{
		MoveLegacyAbsolute(m.id, m.targetPos);
	}
	...
	for (auto& m : motors)
	{
		...
		if (!WaitForMotorMotion(m.id, (float)timeoutMs))
		{
...
void CKongTan8dianjiDlg::OnBnClickedBtnProportionalHook()
{
	...
	// 联动电机1-4
	if (SetLegacyVelocity(1, 1.5 / 1.1 * motor_V56, 200000, 200000))
	{
		MotorMoveRel(1, 1.5 / 1.1 * motor_S56);
	}
	if (SetLegacyVelocity(2, 4.25 / 1.5 * motor_V56, 200000, 200000))
	{
		MotorMoveRel(2, -4.25 / 1.5 * motor_S56);
	}
	if (SetLegacyVelocity(3, 2.75 / 1.5 * motor_V56, 200000, 200000))
	{
		MotorMoveRel(3, -2.75 / 1.5 * motor_S56);
	}
	if (SetLegacyVelocity(4, 0.34 / 1.5 * motor_V56, 200000, 200000))
	{
		MotorMoveRel(4, -0.34 / 1.5 * motor_S56);
	}
	...
	double wr)
{
	...
	for (int i = 0; i < num_steps; i++)
	{
		...
		for (int k = 0; k < 6; k++) {
			...
			SetLegacyVelocity(id, speed, 200000, 200000);
		}
		...
		for (int k = 0; k < 6; k++) {
			...
			if (timeoutMs > 60000.0) timeoutMs = 60000.0;
			if (!WaitForMotorMotion(id, (float)timeoutMs)) return;
		}
	}
