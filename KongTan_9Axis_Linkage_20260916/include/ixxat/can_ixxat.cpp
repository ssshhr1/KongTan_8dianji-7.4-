...
const Error *IxxatCAN::Open( void )
{
    ...
    * Check for "select" first - if the port name is
    * "select", show a dialog to pick the hardware.
    **************************************************/
   const char *portName = GetName();
   int selectDialog = 0;
   XAT_BoardCFG cfg;
   HRESULT res;
   DWORD key = 0;
   ...
   if( portName && (_stricmp( portName, "select" ) == 0) )
   {
      selectDialog = 1;
   }
   else
    ...
    * port name specified.  If "select" was chosen,
    * show the IXXAT hardware selection dialog.
    **************************************************/
   if( selectDialog )
   {
      // Show the IXXAT hardware selection dialog box
      memset( &cfg, 0, sizeof(cfg) );
      int dlgResult = lpXAT_SelectHardware( NULL, &cfg );
      if( dlgResult <= 0 )
      {
         if( dlgResult == 0 )
            err = &CanError::BadPortName;   // User cancelled dialog
         else
            err = &CanError::Driver;        // Dialog error
         goto done;
      }
      // cfg.board_type and cfg.board_no are now set from the dialog
   }
   else
   {
      key = port;
